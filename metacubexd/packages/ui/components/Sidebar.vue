    ...
    <div v-if="route.path !== '/setup'" class="drawer-side z-[60]">
      ...
      >
        ...
        >
          ...
          <!-- 订阅转换（sub-web）快捷入口，新标签页打开 -->
          <div
            class="mb-2 hidden lg:flex"
            :class="configStore.sidebarExpanded ? '' : 'justify-center'"
          >
            <a
              :href="subWebUrl"
              target="_blank"
              rel="noopener noreferrer"
              class="press-tactile flex h-9 cursor-pointer items-center justify-center gap-1.5 rounded-lg border border-[var(--sidebar-border)] bg-transparent text-xs font-medium text-base-content no-underline hover:border-[color-mix(in_oklab,var(--color-primary)_40%,transparent)] hover:bg-[color-mix(in_oklab,var(--color-primary)_12%,transparent)] hover:text-primary"
              :class="
                configStore.sidebarExpanded ? 'w-full' : 'aspect-square w-9'
              "
              title="订阅转换"
            >
              <IconLink class="h-4 w-4 shrink-0" />
              <span v-if="configStore.sidebarExpanded" class="truncate">
                订阅转换
              </span>
            </a>
